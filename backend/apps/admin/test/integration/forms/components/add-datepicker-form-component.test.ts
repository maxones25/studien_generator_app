...
describe('create DatePicker form component', () => {
  let app: IApp;
  ...
  beforeAll(async () => {
    ...
    fieldId = await createFieldId(app, {
      ...
      studyId,
      entityId,
      data: { name: fakeData.text(), type: FieldType.Date },
    });
  ...
  it('should add DatePicker form component', async () => {
    const type = ComponentType.DatePicker;
    const attributes = {
      required: true,
      defaultValue: datetime.formatDate(fakeData.futureDate()),
      label: fakeData.text(),
    };
  ...
  it('should add with defaultValue CurrentDate', async () => {
    const type = ComponentType.DatePicker;
    const attributes = {
      required: true,
