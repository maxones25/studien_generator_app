...
describe('create TimePicker form component', () => {
  let app: IApp;
  ...
  beforeAll(async () => {
    ...
    fieldId = await createFieldId(app, {
      ...
      studyId,
      entityId,
      data: { name: fakeData.text(), type: FieldType.Time },
    });
  ...
  it('should add TimePicker form component', async () => {
    const type = ComponentType.TimePicker;
    const attributes = {
      required: true,
      defaultValue: datetime.formatTime(fakeData.futureDate()),
      label: fakeData.text(),
    };
  ...
  it('should add with defaultValue CurrentTime', async () => {
    const type = ComponentType.TimePicker;
    const attributes = {
      required: true,
