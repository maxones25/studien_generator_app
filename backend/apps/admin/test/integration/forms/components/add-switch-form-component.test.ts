...
describe('create Switch form component', () => {
  let app: IApp;
  ...
  beforeAll(async () => {
    ...
    fieldId = await createFieldId(app, {
      ...
      studyId,
      entityId,
      data: { name: fakeData.text(), type: FieldType.Boolean },
    });
  ...
  it('should add Switch form component', async () => {
    const type = ComponentType.Switch;
    const attributes = { required: true, defaultValue: true, label: 'test' };
